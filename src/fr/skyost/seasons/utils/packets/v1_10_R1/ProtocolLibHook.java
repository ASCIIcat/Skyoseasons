...
public class ProtocolLibHook extends AbstractProtocolLibHook {
	...
	public ProtocolLibHook(final Plugin protocolLib, final Plugin skyoseasons)
	...
	protected final void translateMapChunk(final PacketContainer packet, final Player player, final Season season) {
		...
		if (d != null) {
			translateChunkInfo(new ChunkInfo(player, packet.getIntegers().read(2), 0,
					getOrDefault(packet.getBooleans().readSafely(0), true), d, 0), season);
		}
	}
